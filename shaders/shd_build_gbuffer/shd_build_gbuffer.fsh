...
uniform int u_iCompatability;	// If < 0, not compat. If >= 0, specifies compat channel that would normally be used
...
void main()
{
    ...
    // NORMALS == 
    if (u_iCompatability < 0 || u_iCompatability == 1){
	    ...
	    gl_FragData[u_iCompatability < 0 ? 1 : 0] = vec4(vNormal.xyz * 0.5 + 0.5, 1.0);
    }
    
    // PBR DATA ==
    if (u_iCompatability < 0 || u_iCompatability == 2){
	...
	/// @stub	Allow storing specular in R channel; however some exports are filling it with garbage so we need a special toggle somewhere
	        gl_FragData[u_iCompatability < 0 ? 2 : 0] = vec4(vec3(1.0, texture2D(u_sPBR, v_vTexcoord).gb) * u_vPBR, 1.0);
	    }
	    else
	        gl_FragData[u_iCompatability < 0 ? 2 : 0] = vec4(u_vPBR, 1.0);
    }

	// EMISSION ==
	if (u_iCompatability < 0 || u_iCompatability == 3){
		if (u_iSamplerToggles[3] > 0){
			vec4 vEmission = texture2D(u_sEmissive, v_vTexcoord);
			gl_FragData[u_iCompatability < 0 ? 3 : 0] = vec4(to_rgb(vEmission.rgb) * u_vEmissive, vEmission.a);
		}
		else
			gl_FragData[u_iCompatability < 0 ? 3 : 0] = vec4(0, 0, 0, 0);
	}
}
