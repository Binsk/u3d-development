		...
		if (is_hovered){
			...
			if (mouse_check_button_pressed(mb_left)){
				if (animation_tree.get_animation_layer_track_name(0) == names[i])
					...
					if (animation_tree.get_animation_layer_exists(0) and obj_demo_controller.animation_smooth)
						animation_tree.queue_animation_layer_transition(0, names[i], 0.25);
					else{
						animation_tree.add_animation_layer_auto(0, names[i], obj_demo_controller.animation_speed);
						animation_tree.start_animation_layer(0, obj_demo_controller.animation_loop);
					}
				}
			}
		}
