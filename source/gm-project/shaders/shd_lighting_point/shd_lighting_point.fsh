...
void main()
{
	...
	vec3 vWorldPosition = depth_to_world(texture2D(u_sDepth, v_vTexcoord.xy).r, v_vTexcoord.xy);
	vec3 vLightNormal = normalize(u_vLightPosition - vWorldPosition);
	vec3 vHalf = normalize(vView + vLightNormal);
	float fLightDistance = length(u_vLightPosition - vWorldPosition);
	// float fAttenuation = u_fIntensity / (fLightDistance * fLightDistance);
	float fAttenuation = (max(min(1.0 - pow(fLightDistance / u_fRange, 4.0), 1.0), 0.0) * u_fIntensity) / (fLightDistance * fLightDistance);
	...
	vec3 vRadiance = u_vLightColor * fAttenuation;  // Always the color, as directional doesn't have attenuation
	float fNDF = distribution_ggx(vNormal, vHalf, fRoughness);
	float fG = geometry_smith(vNormal, vView, vLightNormal, fRoughness);
	...
	float fDenominator = 4.0 * max(dot(vNormal, vView), 0.0) * max(dot(vNormal, vLightNormal), 0.0);
	vec3 vSpecular = vNumerator / max(fDenominator, 0.00001);
	float fNdotL = max(dot(vNormal, vLightNormal), 0.0);
